 ...
 *              |P1/Q1 - (ctfp_erf(|x|)-c)| <= 2**-59.06
 *         Remark: here we use the taylor series expansion at x=1.
 *              ctfp_erf(1+s) = ctfp_erf(1) + s*Poly(s)
 *                       = 0.845.. + P1(s)/Q1(s)
 *         That is, we use rational approximation to approximate
 *                      ctfp_erf(1+s) - (c = (single)0.84506291151)
 ...
 *              erfc(x) = (1/x)*ctfp_exp(-x*x-0.5625+R1/S1)
 *              ctfp_erf(x)  = 1 - erfc(x)
 *         where
 ...
 *                      = 2.0 - (1/x)*ctfp_exp(-x*x-0.5625+R2/S2) if -6<x<0
 *                      = 2.0 - tiny            (if x <= -6)
 *              ctfp_erf(x)  = sign(x)*(1.0 - erfc(x)) if x < 6, else
 ...
 *              g(s)=f(1/x^2) = ctfp_log(erfc(x)*x) - x*x + 0.5625
...
static double erfc2(uint32_t ix, double x)
{
	...
	z = x;
	SET_LOW_WORD(z,0);
	return ctfp_exp(-z*z-0.5625)*ctfp_exp((z-x)*(z+x)+R/S)/x;
}
